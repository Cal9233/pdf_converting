...
class SimplePDFConverter:
    ...
    def parse_chase_pdf(self, pdf_path):
        """Parse Chase PDF with multiple cardholder support."""
        transactions = []
        validation_errors = []
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if pdf.pages:
                ...
                if text:
                    ...
                    for line in lines[:50]:  # Check first 50 lines
                        ...
                        if line in VALID_CARDHOLDERS:
                            primary_cardholder = line
                            current_cardholder = line
                            break
            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                for i, line in enumerate(lines):
                    ...
                    # Check for cardholder sections: "NAME" followed by "TRANSACTIONS THIS CYCLE"
                    if 'TRANSACTIONS THIS CYCLE' in line.upper() and 'CARD' in line.upper():
                        # Check if previous line is a cardholder name
                        if i > 0:
                    ...
                    # Also check if name and TRANSACTIONS THIS CYCLE are on same line
                    ...
                    # Check for standalone cardholder names (2-4 words, all caps)
                    ...
                    # Parse transaction - skip negative amounts and payments
                    match = re.match(r'^(\d{2}/\d{2})\s+(.+?)\s+([-]?\$?[\d,]+\.\d{2})$', line_stripped)
                    if match:
                        ...
                        date_str = match.group(1) + '/24'  # Add year
