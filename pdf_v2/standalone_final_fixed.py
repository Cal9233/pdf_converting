...
class SimplePDFConverter:
    ...
    def parse_chase_pdf(self, pdf_path):
        """Parse Chase PDF - Fixed to find cardholder from 'Transactions of this cycle'."""
        transactions = []
        validation_errors = []
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                # First pass: Find the primary cardholder
                for page in pdf.pages:
                    ...
                    # Look for "Transactions of this cycle" section
                    for i, line in enumerate(lines):
                        if 'Transactions of this cycle' in line:
                            # Look at next few lines for cardholder name
                            for j in range(i + 1, min(i + 5, len(lines))):
                                next_line = lines[j].strip()
                                # Check against valid cardholders
                                for valid_name in VALID_CARDHOLDERS:
                                    if valid_name in next_line.upper():
                                        primary_cardholder = valid_name
                                        break
                                if primary_cardholder:
                                    break
                        if primary_cardholder:
                            break
                    if primary_cardholder:
                ...
                # Default to LUIS RODRIGUEZ if not found (as you mentioned it should be)
                if not primary_cardholder:
                    primary_cardholder = 'LUIS RODRIGUEZ'
                    validation_errors.append("Could not find cardholder name, defaulting to LUIS RODRIGUEZ")
                ...
                for page_num, page in enumerate(pdf.pages):
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if date_match:
                            ...
                            if amount_match:
                                amount_str = amount_match.group(1).replace('-', '')  # Remove negative
            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                for form_idx, (ssn_line, ssn) in enumerate(ssn_positions):
                    ...
                    for i, line in enumerate(form_lines):
                        ...
                        # Employer name (Box c) - FIXED: Extract from same line
                        if "employer's name" in line.lower():
                            # First check if employer name is on the same line
                            line_parts = line.split()
                            employer_found = False
                            
                            # Look for known employer pattern
                            if 'Ocomar' in line:
                                ...
                                if i + 1 < len(form_lines):
                                    ...
                                    if 'Ocomar' in next_line:
                                        ...
                                        # Extract company name before any numbers
