#!/usr/bin/env python3
"""
Fixed GUI version with just a progress window
Uses current working directory like original_program.py
"""
...
class StandalonePDFConverter:
    ...
    def _save_to_excel(self, transactions: list, subdir: str):
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            for col, width in column_widths.items():
                if col <= chr(65 + len(df.columns) - 1):  # Check if column exists
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages):
                ...
                for form_idx, (ssn_line, ssn) in enumerate(ssn_positions):
                    ...
                    for i, line in enumerate(form_lines):
                        ...
                        if "employee's first name" in line.lower():
                            if i + 1 < len(form_lines):
                                ...
                                for word in words:
                                    # Stop if we hit a number or certain keywords
                    ...
                    # Add to results if we have at least some data
    ...
    def _save_w2_to_excel(self, w2_data: list):
        ...
        # Create DataFrame with W2 column order
